...
2.  Создать класс вещественных матриц (вариант вычисляется по формуле  $(n - 1)\bmod 3 + 1$, где $n$ – номер в журнале):

**Вариант 1** – несимметричные разреженные матрицы с несимметричным портретом, формат хранения CSR  [1] (класс `CSRMatrix`):
...
**Вариант 2** – несимметричные разреженные матрицы с симметричным портретом, формат хранения CSLR [1] (класс `CSLRMatrix`):
*	требования те же, что и для варианта 1.

**Вариант 3** – симметричные разреженные матрицы с симметричным портретом, формат хранения CSLR [1] (класс `CSLRMatrix`):
*	требования те же, что и для варианта 1.
...
$$
{\bf{\vec r}} =  - \alpha {\bf{A}} \cdot \left( {{\bf{\vec x}} - \beta {\bf{\vec y}}} \right) + {\bf{\vec z}}\gamma .
$$
